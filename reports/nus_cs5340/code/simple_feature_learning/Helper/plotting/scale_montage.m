function [h,bigImage] = scale_montage(varargin)
...
if isempty(cmap)
    ...
    % keyboard
    if (nargout<2)
%         hh = imagesc(bigImage, displayRange); colorbar;
        hh = imshow(bigImage,'Border','tight');
    end
else
    if (nargout<2)
%         hh = imagesc(bigImage,cmap);  colorbar;
                hh = imshow(bigImage,'Border','tight');
    end
end
