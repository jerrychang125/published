%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%>
% The entry point to train a 2 layer Deconvolutional Network jointly (so
% optimizes both first and second layer reconstruction and sparsity
% simultaneously).
...
%
% @training_file @copybrief train_yann.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
end % Setup parameters for both layers at one initialy.

% Set the number of layer separately.
model1.num_layers = 1;
model2.num_layers = 2;



% If you want to train the first layer (these outputs will be input into
% the 2-layer Yann's model below.
if(guimodel.LAYER1_FIRST)
   [F1,z1,z01,y_tilda1_1lay] = train_recon_layer(model1,F1,z01,y,original_images,'train');
else
    z1 = 0;
end
...
for layer=model.num_layers:-1:1
...
% Added z1 and z01 feature maps to the inputs.
modelargs = strcat(modelargs,',z1,y,original_images,',char(39),'train',char(39));


eval(strcat('[F1,F2,z1,z2,z0,y_tilda1,y_tilda2] = train_recon_yann(',modelargs,');'));
