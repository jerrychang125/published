function [] = ITSBNImageSegm2(filename_input,...
                                          imagename, imageext,...
                                          C,...
                                          iteration_max,...
                                          exp_number,...
...
% ###############################################################
% ###############################################################
% The biggest change from 2.0.2 is that 2.1.0 does not use H(0), so we
% don't have to make identity matrix CPT between H(0) and H(1). 
% That is because we know that any discrete node can be soft evidence  

% =============================================================
% by Kittipat "Bot" Kampa
...
if nargin < 4 C{1+1,2} = 0; end % number of classes/mixtures in GMM
...
fig_ITSBN_boundary = 109;
fig_ITSBN_display = 110;
...
%Y = Y(:,[1:6,27:35, 36:75]); % fesature learning
...
if C{1+1,2} == 0 % We will use BIC to decide the number of clutster
    BIC_winner = -1e+9;
    for c = 3:5
    ...
    gmm_obj = gmdistribution.fit(Y,C{1+1,2},'Regularize',1e-4,'Replicates',20,'Options',options);
end

mu_c = gmm_obj.mu; % zeros(C,D);
Sigma_c = gmm_obj.Sigma; % zeros(D,D,C);
Lambda_c = zeros(D,D,C{1+1,2});
for c = 1:C{1+1,2}
...
CPT{0+1,2} = eye(C{0+1,2}); % normally H0 does not need CPT, but I just put it there just in case.
for l = 1:(L-2)
    CPT{l+1,1} = l;
    cpt = fn_pseudoDiag(C{l+1,2},C{l+1+1,2}) + cpt_spread_factor *rand(C{l +1,2},C{l+1 +1,2}); % 0.9 --> very unspecific CPT
%     cpt = eye(C{l+1,2})+ cpt_spread_factor *rand(C{l+1,2},C{l+1,2}); % 0.9 --> very unspecific CPT
    cpt = cpt./repmat(sum(cpt,1),C{l+1,2},1);
    CPT{l+1,2} = cpt;
end
CPT{L-1+1,1} = L-1;
CPT{L-1+1,2} = ones(1, C{L-1+1,2})/C{L-1+1,2};
...
% In this version, node in different level has different number of class
node_sizes = zeros(1,K); 
for l = L-1:-1:1
    node_sizes(1,H{l+1,2}) = C{l+1,2};
end
% make bnet
...
script_processImportantInfo;
save(imagename,'segs','posterior_marg');
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
movefile(['./',imagename,'.mat'], ['./',imagename, '/', imagename,'.mat']);
% =========================================================================
